...
-->
<xsl:transform exclude-result-prefixes="eg html rng tei epub" extension-element-prefixes="exslt" version="1.1" xmlns="http://www.w3.org/1999/xhtml" xmlns:eg="http://www.tei-c.org/ns/Examples" xmlns:epub="http://www.idpf.org/2007/ops" xmlns:exslt="http://exslt.org/common" xmlns:html="http://www.w3.org/1999/xhtml" xmlns:rng="http://relaxng.org/ns/structure/1.0" xmlns:tei="http://www.tei-c.org/ns/1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
...
-->
  ...
  <xsl:key match="tei:note" name="note-pb" use="generate-id(  preceding::*[self::tei:pb[not(@ed)][@n] ][1] ) "/>
      ...
      <xsl:otherwise>
        ...
        <html>
          ...
          <head>
            ...
            <meta content="{$date}" name="modified"/>
            ...
            -->
            <link charset="utf-8" href="Teinte/tei2html.css" rel="stylesheet" type="text/css"/>
            <script charset="utf-8" src="Teinte/Tree.js" type="text/javascript">//</script>
          </head>
          <body class="{$corpusid}">
            <div id="center">
              <div id="main">
            ...
            <script charset="utf-8" src="dialog.js" type="text/javascript">//</script>
          </body>
        </html>
      </xsl:otherwise>
    </xsl:choose>
...
(group/text)
  ...
  <xsl:template match="tei:text">
    ...
    <article>
      ...
      <xsl:call-template name="atts"/>
      <div class="sp" id="init" style="background-color:pink;margin:-2px -20px;padding:2px 20px;position:relative;height:30px;">Réplique interrompant un dialogue entre deux personnages
        <div class="navigation
          init" style="position:absolute;right:5px;top:3px;">
...
<h3>Paragraphs</h3>
  ...
  <!--MARC
  reste à indiquer les changements / les répliques pivots
  -->
  <xsl:template match="tei:sp[(@who != preceding::tei:sp[2]/@who) and (@who != following::tei:sp[2]/@who) and (@who != preceding::tei:sp[1]/@who) and (@who != following::tei:sp[1]/@who)]">
    <!--        <xsl:variable name="previousId" select="//preceding::tei:sp[(@who != preceding::tei:sp[2]/@who) and (@who != following::tei:sp[2]/@who) and (@who != preceding::tei:sp[1]/@who) and (@who != following::tei:sp[1]/@who)]/@xml:id"/>
    <xsl:variable name="nextId" select="//following::tei:sp[(@who != preceding::tei:sp[2]/@who) and (@who != following::tei:sp[2]/@who) and (@who != preceding::tei:sp[1]/@who) and (@who != following::tei:sp[1]/@who)]/@xml:id"/>
-->
    <div style="background-color:pink;margin:-2px -20px;padding:2px 20px;position:relative;">
      <xsl:call-template name="atts"/>
      <xsl:attribute name="id">
        <xsl:call-template name="id"/>
      </xsl:attribute>
      ...
      <div style="position:absolute;right:5px;top:3px;">
        <xsl:attribute name="class">navigation <xsl:call-template name="id"/> </xsl:attribute>
        <div class="arrow-up">
          <a>&#8593;</a>
        </div>
        <div class="arrow-down">
        ...
        <div>
          ...
          <xsl:if test="@part = 'M' or @part = 'm' or @part = 'F' or @part = 'f'  or @part = 'y'  or @part = 'Y'">
            <xsl:variable name="txt">
              <xsl:apply-templates mode="lspacer" select="preceding::tei:l[ancestor::tei:body][1]"/>
            </xsl:variable>
  ...
  <xsl:template match="tei:l" mode="lspacer">
    ...
    <xsl:choose>
      ...
      <xsl:when test="@part = 'F'">
        <xsl:apply-templates mode="lspacer" select="preceding::tei:l[1]"/>
        <xsl:value-of select="$txt"/>
        <xsl:text> </xsl:text>
      </xsl:when>
    ...
    <xsl:variable name="notes">
      ...
      <xsl:choose>
        ...
        <xsl:when test="$cont and function-available('exslt:node-set')">
          <xsl:apply-templates mode="fn" select="exslt:node-set($cont)">
...
<h3>Apparat critique</h3>
  ...
  <xsl:template match="tei:space">
    <xsl:choose>
      ...
      <xsl:otherwise>
        <span class="space" style="width:2em;">    </span>
      </xsl:otherwise>
    </xsl:choose>
    <xsl:apply-templates/>
  </xsl:template>
  ...
  <xsl:template name="anchors">
    ...
    <xsl:choose>
      ...
      <xsl:when test="key('id', substring-after($id, '#'))">
        <xsl:text> </xsl:text>
        <xsl:apply-templates mode="a" select="key('id', substring-after($id, '#'))"/>
      </xsl:when>
